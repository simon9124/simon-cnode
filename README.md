...
- mpvue-wxParse 小程序富文本：默认将换行符清空，手动设置为<br>标签又包含高度，因此用一个看不见的<hr>代替：
...
- mpvue-wxParse 小程序富文本：解析 a 标签跳转问题，用@navigate 实现点击<a>标签的事件，再用 web-view 组件实现页面跳转，且需在小程序后台**配置业务域名**方可正常跳转。因个人和海外账号不可配置业务域名，本项目暂无法实现外链跳转。开发时，可**取消域名校验**进行相关测试
